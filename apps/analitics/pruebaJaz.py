...
def display_value_two(grupo_chosen):
    ...
    poblacion=pd.read_sql('SELECT * FROM poblacion', conn)## conection to poblacion table to poblacion DataFrame for calculate the verified coverage rate.
    malnutrition['nombre_comuna']=salud['nombre_comuna'].replace('nan', np.NaN) # To assing NaN values to 'nan' text strig for select with NaN. 
